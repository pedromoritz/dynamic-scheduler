...
test()
{
  ...
  # starting testset
  k6 run -q --out csv="results/results_${ST}_${PA}_${RR}.gz" -e SVC_IP=$SVCIP -e SCHEDULER_TYPE=$ST -e POD_AMOUNT=$PA -e REQUESTS_RATE=$RR k6_script-ramp.js >/dev/null 2>&1 &
